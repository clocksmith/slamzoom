...
public class BitmapSet {
  ...
  private int[] mSizes = new int[BitmapUtils.NUM_BITMAPS_IN_SET];
  private Bitmap[] mBitmaps = new Bitmap[BitmapUtils.NUM_BITMAPS_IN_SET];
  private float mAspectRatio;

  public BitmapSet(Context context, Uri imageUri, int startSize) {
    int size = startSize;
    for (int i = 0; i < BitmapUtils.NUM_BITMAPS_IN_SET; i++) {
      try {
        Bitmap bitmap;
