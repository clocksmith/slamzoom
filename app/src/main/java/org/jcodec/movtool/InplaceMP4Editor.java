...
public class InplaceMP4Editor {
    ...
    public boolean modify(File file, MP4Edit edit) throws IOException {
        ...
        try {
            ...
            List<_2<Atom, ByteBuffer>> fragments = doTheFix(fi, edit);
            if (fragments == null)
                return false;

            // If everything is clean, only then actually writing stuff to the
            // file
            for (_2<Atom, ByteBuffer> fragment : fragments) {
    ...
    private List<_2<Atom, ByteBuffer>> doTheFix(SeekableByteChannel fi, MP4Edit edit) throws IOException {
        ...
        List<_2<Atom, ByteBuffer>> fragments = new LinkedList<_2<Atom, ByteBuffer>>();
        if (BoxUtil.containsBox(moovBox, "mvex")) {
            List<_2<ByteBuffer, MovieFragmentBox>> temp = new LinkedList<_2<ByteBuffer, MovieFragmentBox>>();
            ...
            for (_2<ByteBuffer, ? extends Box> frag : temp) {
